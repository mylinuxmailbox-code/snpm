...
export async function runResolve(root: string, flags: ResolveFlags): Promise<number> {
  ...
  const onEvent = (e: RegistryEvent): void => {
    switch (e.kind) {
      case 'fetched':
        fetched += 1;
        spinner?.update(`resolving ${c.dim(e.name)} (${fetched} fetched, ${cached} cached)`);
        return;
      case 'cache-hit':
      case 'revalidated':
        cached += 1;
        return;
      case 'stale-fallback':
        securityWarn('STALE_METADATA', { pkg: e.name, ageMin: Math.round(e.ageMs / 60_000), cause: JSON.stringify(e.cause) });
        return;
      case 'metadata-warning':
        securityWarn('MALFORMED_METADATA', { pkg: e.error.pkg, reason: JSON.stringify(e.error.reason) });
        return;
    }
  };
  ...
  const registry = new RegistryClient(
    new HttpTransport(),
    new BoundedPool(config.limits.metaConcurrency),
    new FsMetaStore(layout.metaCache),
    systemClock,
    { ...DEFAULT_REGISTRY_OPTIONS, registry: config.registry },
    onEvent,
  );
  const resolver = new Resolver(registry, undefined, { ...DEFAULT_RESOLVER_OPTIONS, includeDev: !flags.production });
  ...
  try {
    ...
    if (flags.json) {
      const nodes = [...graph.nodes.values()].map((n) => ({ ...n, dependencies: Object.fromEntries(n.dependencies) }));
      process.stdout.write(`${JSON.stringify({ roots: Object.fromEntries(graph.roots), nodes, warnings: graph.warnings })}\n`);
    }
    return 0;
  } catch (err: unknown) {
    spinner?.stop();
